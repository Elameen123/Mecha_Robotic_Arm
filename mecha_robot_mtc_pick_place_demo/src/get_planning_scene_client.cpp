...
#include "mecha_robot_mtc_pick_place_demo/get_planning_scene_client.h"
...
: Node("get_planning_scene_client")
{
  ...
  client_ = create_client<mecha_robot_interfaces::srv::GetPlanningScene>("/get_planning_scene_mycobot");
}
...
GetPlanningSceneClient::call_service(const std::string& target_shape, const std::vector<double>& target_dimensions)
{
  ...
  // Prepare the request
  auto request = std::make_shared<mecha_robot_interfaces::srv::GetPlanningScene::Request>();
